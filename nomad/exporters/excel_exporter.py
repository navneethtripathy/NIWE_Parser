...
def _generate_metadata_lines(metadata: LoggerMetadata, stream: Optional[RecordStream]=None, timezone_label: Optional[str]=None) -> list:
    info = metadata.logger_info
    tz = timezone_label or info.time_zone or 'UTC'
    lines = [f"Site name: {info.site_name or ''}", f"Site description: Serial number: {info.serial_number or ''}", f"Latitude: {info.latitude or ''}", f"Longitude: {info.longitude or ''}", f'Elevation: {(int(info.elevation_m) if info.elevation_m else 0)} m', f'Time zone: {tz}', f"Logger Model: {info.logger_model or ''}", f"Serial Number: {info.serial_number or ''}", '', 'Included flags: <Unflagged data>', 'Excluded flags: ', '', 'Time stamps indicate the beginning of the time step.', '']
    return lines
...
def export_workbook(stream: RecordStream, metadata: LoggerMetadata, base_output_path: Path, timezone_label: Optional[str]=None) -> List[Path]:
    ...
    if '1min' in freq_groups:
        ...
        df_1m = pd.DataFrame({ts_col_name: unpacked_ts, col_name: unpacked_vals})
