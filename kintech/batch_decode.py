...
def decode_wnd_files(input_dir: Path, output_dir: Path, output_format: str='raw') -> List[Path]:
    ...
    for wnd_path in wnd_files:
        try:
            ...
            parsed = reader.parse()
            transformer = RecordTransformer(parsed, output_style=output_format, resample=output_format == 'windographer', interval_minutes=10, timestamp_shift_minutes=10 if output_format == 'windographer' else 0, fill_gaps=output_format == 'windographer')
...
def move_to_processed(files: List[Path], processed_dir: Path) -> None:
    if not files:
        return
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog='batch_decode.py', description='End-to-end Kintech data processing pipeline: decode → visualize → report → PDF.')
    BASE_DIR = Path(__file__).resolve().parent
    parser.add_argument('--input-dir', type=Path, default=BASE_DIR / 'input', help='Directory containing raw .wnd files (default: ./input).')
    parser.add_argument('--output-dir', type=Path, default=BASE_DIR / 'output', help='Directory for decoded CSV output (default: ./output).')
    parser.add_argument('--processed-dir', type=Path, default=BASE_DIR / 'processed', help='Directory to move processed .wnd files (default: ./processed).')
    parser.add_argument('--viz-dir', type=Path, default=BASE_DIR / 'visualizations', help='Directory for visualizations (default: ./visualizations).')
    parser.add_argument('--report-dir', type=Path, default=BASE_DIR / 'reports', help='Directory for reports and PDFs (default: ./reports).')
    parser.add_argument('--format', choices=['raw', 'windographer'], default='raw', help='Output column layout for decoding (default: raw).')
    parser.add_argument('--skip-decode', action='store_true', help='Skip decoding stage — only run visualization/reporting on existing outputs.')
    parser.add_argument('--no-move', action='store_true', help="Don't move .wnd files to processed/ after decoding.")
...
def main(argv=None) -> int:
    ...
    if not args.skip_decode:
        ...
        decoded_files = decode_wnd_files(args.input_dir, args.output_dir, args.format)
        if decoded_files:
    ...
    if not args.no_pdf:
        ...
        for result in results:
            generate_pdf(dataset_name=result.dataset_name, viz_dir=result.viz_dir, report_dir=result.report_dir, output_pdf_dir=args.report_dir, alpha=result.alpha, summary_text=result.summary_text, quality_text=result.quality_text, statistics_text=result.statistics_text)
        print('└────────────────────────────────────────────────────────────┘')
    else:
